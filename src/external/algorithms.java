...
public class algorithms {
	static int num_jobs;
	// size = [num_jobs][2], for every job [0] is the length, [1] is the due
	// time
	static int jobs[][];
	...
	/**
	 * Greedy and best_first use the same memory areas for storing jobs
	 * @return
	 */
	static class GreedyMinTar implements MinTar{
		schedule s;
		@Override
		...
		public void setJobs(List<Job> jobs) {
			algorithms.num_jobs = jobs.size();
			algorithms.jobs = new int[algorithms.num_jobs][2];
			Job j;
			for(int i=0; i<jobs.size(); i++){
				j = jobs.get(i);
				algorithms.jobs[i][0] = j.getDuration();
				algorithms.jobs[i][1] = j.getDue();
				//[num_jobs][2], for every job [0] is the length, [1] is the due
	 ...
	 * @return
	 */
	static class BestFirstMinTar implements MinTar{
		schedule s;
		@Override
		public String getLabel() {
			// TODO Auto-generated method stub
	...
	public static MinTar greedy(){
		return new GreedyMinTar();
	}
	
	public static MinTar bestfirst(){
		return new BestFirstMinTar();
	}
